...
import { ArrowLeft, Info, CheckCircle2, XCircle, ChevronUp, ChevronDown } from "lucide-react";
...
const UnlockPi = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      {/* Compact Header Image with Navigation */}
      <div className="relative w-full overflow-hidden bg-[#6C3F99]">
        <img 
          src="./images/header 1.jpg" 
          alt="Unlock Pi Header" 
          className="w-full h-10 sm:h-12 object-contain opacity-90"
          style={{ imageRendering: 'crisp-edges' }}
        />
        {/* Purple overlay for consistency */}
        <div className="absolute inset-0 bg-[#6C3F99]/20 pointer-events-none"></div>
        {/* Back button overlay */}
        <div className="absolute top-1.5 left-2.5 sm:top-2 sm:left-3 z-10">
          <ArrowLeft 
            className="w-4 h-4 sm:w-5 sm:h-5 text-white cursor-pointer hover:text-pi-gold transition-colors drop-shadow-lg" 
            onClick={handleBack} 
          />
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5 max-w-lg mx-auto">
        {/* Information Banner - Professional Design */}
        <div className="bg-gradient-to-r from-amber-50 to-yellow-50 border border-pi-gold/30 rounded-xl p-3 sm:p-4 flex items-start space-x-3 shadow-sm">
          <Info className="w-4 h-4 sm:w-5 sm:h-5 text-pi-purple mt-0.5 flex-shrink-0" />
          <p className="text-xs sm:text-sm text-amber-900 leading-relaxed">
            You can unlock the Pi you are locked in. Please select the amount of Pi you want to unlock.
          </p>
        </div>

        {/* Select Wallet Button - Professional Design */}
        <Button 
          onClick={handleSelectWallet}
          className="w-full bg-gradient-to-r from-pi-purple to-purple-700 hover:from-purple-700 hover:to-purple-800 active:scale-98 text-white py-3 sm:py-3.5 text-sm sm:text-base font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
        >
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            />
            {walletAddress.length > 0 && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-pi-purple">
                {isValidAddress ? (
        ...
        <div className="bg-gray-100 rounded-lg overflow-hidden border border-gray-300">
          ...
          >
            ...
            <div className="bg-gray-300 rounded-full p-1">
              {showFAQ ? (
                <ChevronUp className="w-4 h-4 text-pi-purple" />
              ) : (
                <ChevronDown className="w-4 h-4 text-pi-purple" />
              )}
            </div>
          </div>
        ...
        {/* Unlock Request Button - Professional Design */}
        <Button 
          onClick={handleUnlockRequest}
          disabled={lockedBalance > 0 && piAmount[0] > Math.floor(lockedBalance * 0.5)}
          className={`w-full py-3 sm:py-3.5 text-sm sm:text-base font-bold transition-all duration-200 shadow-lg ${
            lockedBalance > 0 && piAmount[0] > Math.floor(lockedBalance * 0.5)
